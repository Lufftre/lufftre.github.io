import"./modulepreload-polyfill-9p4a8sJU.js";const g=`Game 1: 1 green, 2 red, 6 blue; 4 red, 1 green, 3 blue; 7 blue, 5 green; 6 blue, 2 red, 1 green
...
Game 100: 4 green, 4 blue, 15 red; 3 green, 1 red, 13 blue; 5 green, 5 blue, 10 red`;document.querySelector("#input").innerHTML=g;function d(e,r){const u=document.querySelectorAll(".output")[e];return u.innerHTML+=r+`
`,r}const o=String(g),s={red:12,green:13,blue:14},a=o.split(`
`).map(e=>{const[r,n]=e.split(":"),u=parseInt(r.split(" ")[1]),b=n.split(";").map(l=>l.split(",").map(m=>{const[G,t]=m.split(" ").slice(1);return[parseInt(G),t]}));return[u,b]}),c=a.reduce((e,[r,n])=>{for(const u of n)for(const[b,l]of u)if(s[l]<b)return d(0,"❌"),e;return d(0,"✔️"),e+r},0);console.log(c);const p=a.map(([e,r])=>{const n={};for(const u of r)for(const[b,l]of u)n[l]=Math.max(n[l]??0,b);return Object.values(n)}).map(e=>e.reduce((r,n)=>r*n)).map(e=>d(1,e)).reduce((e,r)=>e+r);console.log(p);
