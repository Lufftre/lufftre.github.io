import"./modulepreload-polyfill-9p4a8sJU.js";const f=`hcpjssql4kjhbcqzkvr2fivebpllzqbkhg
...
7bbfbcvh6
ffnrprtnine1tjznmckv5sixczv`;document.querySelector("#input").innerHTML=f;function h(n,e){const s=document.querySelectorAll(".output")[n];return s.innerHTML+=e+`
`,e}const v=String(f),r=v.replace(/[^\d\n]/g,"").split(`
`).map(n=>n[0]+n.slice(-1)).reduce((n,e)=>n+parseInt(e),0);console.log(r);const i=["one","two","three","four","five","six","seven","eight","nine"],g=v.split(`
`).map(n=>i.reduce((e,t,s)=>e.replaceAll(t,t+(s+1)+t),n)).map(n=>h(0,n)).map(n=>n.replace(/[^\d]/g,"")).map(n=>h(1,n)).map(n=>n[0]+n.slice(-1)).map(n=>h(2,n)).reduce((n,e)=>n+parseInt(e),0);console.log(g);
